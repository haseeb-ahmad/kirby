    ...
    module PagesHelper
      def link_to_add_structure_item_fields(f, &block)
        ...
        fields = f.fields_for(:structure_items, item, child_index: item.object_id) do |builder|
          ...
          render("kirby/admin/structure_items/fields", f: builder)
        end
      ...
      def partable_type_partial_namespace(partable_type)
        partable_type.tableize.sub(/\Akirby\//, '')
      end
      ...
      def page_ancestry_options(page)
        pages = Kirby::Page.active
        pages = page.resource_page? ? pages.where(resource: page.resource).or(pages.regular_pages) : pages.regular_pages
        ...
        flatten_nested_hash(pages.arrange(order: :position)).map do |page|
          next if page.depth >= Kirby.config.max_page_depth - 1
