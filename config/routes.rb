Kirby::Engine.routes.draw do
  # Backend
  namespace :admin, path: Kirby.config.backend_path do
    root to: "pages#index"
  ...
  unless Kirby.config.disable_frontend_routes
    # Frontend
    root to: "pages#homepage"

    # Pages
    get '/:locale/*id' => 'pages#show', constraints: {locale: /#{Kirby.config.locales.join('|')}/ }
    get '/:locale/' => 'pages#homepage', constraints: {locale: /#{Kirby.config.locales.join('|')}/ }
    get '/*id' => 'pages#show', as: "page", controller: 'pages', constraints: lambda { |request|
      !(Rails.env.development? && request.env['PATH_INFO'].starts_with?('/rails/') || request.env['PATH_INFO'].starts_with?("/#{Kirby.config.backend_path}") || request.env['PATH_INFO'].starts_with?('/attachments/'))
    }
  end
